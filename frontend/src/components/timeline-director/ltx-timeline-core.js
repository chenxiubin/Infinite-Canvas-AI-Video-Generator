...
const api = {
  async fetchApi(path, opts) {
    if (path === '/upload/image' && opts && opts.method === 'POST' && opts.body) {
      if (typeof window.__ltxUploadBridge === 'function') {
        const file = opts.body.get('image');
        if (file) {
          try {
            const res = await window.__ltxUploadBridge(file);
            return {
              status: 200,
              async json() { return { name: res.name || res.url || res.filename || file.name, subfolder: res.subfolder || '' }; }
            };
          } catch (err) {
            console.error('Upload bridge failed', err);
          }
        }
      }
...
class TimelineEditor {
  ...
  createDOM() {
    ...
    this.segmentBoundsDisplay.textContent = "开始: - | 结束: -";
    ...
    toggleBtn.textContent = "自定义音频：关";
    toggleBtn.title = "切换自定义音频输出";

    const updateToggleStyle = (isOn) => {
      toggleBtn.textContent = isOn ? "自定义音频：开" : "自定义音频：关";
      if (isOn) {
    ...
    strengthLabel.textContent = "引导强度：";
  ...
  updateUIFromSelection() {
    ...
    if (this.segmentBoundsDisplay) {
      if (seg) {
        ...
        this.segmentBoundsDisplay.textContent = `开始: ${startStr} | 结束: ${endStr}`;
      } else {
        this.segmentBoundsDisplay.textContent = "开始: - | 结束: -";
      }
    }
...
// --- React standalone exports ---
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { TimelineEditor, parseInitial };
} else {
